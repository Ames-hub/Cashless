...
class pylog:
    def __init__(self, filename='logs/%TIMENOW%.log', logform='%loglevel% - %time% - %file% | '):
        ...
        if '%DATENOW%' in filename:
            filename = filename.replace('%DATENOW%', datetime.datetime.now().strftime("%Y-%m-%d"))
    ...
    def parse_logform(self, loglevel):
        ...
        logform = logform.replace('%date%', datetime.datetime.now().strftime("%Y-%m-%d"))
...
def logman():
    ...
    try:
        ...
        print("Waiting for logs to be written to file...")
        try:
            queue = logQueue(cachedir)
            queue.close()
            queue.close(True)
